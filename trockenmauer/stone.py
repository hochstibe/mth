...
from pymesh import Mesh, form_mesh
...
class Geometry:
    ...
    name: str = None
    mesh: 'Mesh' = None
    # vertices: np.ndarray = None  # (n, 3) -> use mesh
    # triangles_index: np.ndarray = None   # indices to the triangle points -> use mesh
    ...
    def calc_triangle_values(self):
        # self.triangles_values = np.array([self.mesh.vertices[j] for t_ind in self.mesh.faces for j in t_ind])
...
class Boundary(Geometry):
    ...
    def __init__(self, x=2., y=0.5, z=1, name='Boundary'):
        ...
        #     [14, 15, 7], [14, 7, 6], [15, 12, 4], [15, 4, 7],  # top
        # ])
        # the mesh is only with the bounding planes (bottom and sides)
    ...
    def __repr__(self):
        return f'<Boundary(vertices={self.mesh.vertices})>'
